...
const changeLinkage = async (req, res) => {
    try{
       await Linkage.findOneAndUpdate({_id: req.body._id},{name: req.body.name, linkages: req.body.linkages},(error,data)=>{
            if(!error){
...
const deleteLinkage = async (req, res) => {
    try{
            await Linkage.findOneAndRemove({_id: req.body._id},(error,deletedRecord)=>{
